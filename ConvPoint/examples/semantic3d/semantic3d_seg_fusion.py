...
def main():
    ...
    net_noc.load_state_dict(torch.load(os.path.join(args.model_noc, "state_dict.pth"), map_location=device))
    #net_rgb.cuda()
    #net_noc.cuda()
    net_rgb.eval()
    net_noc.eval()
    ...
    if args.test:
        net_fusion.load_state_dict(torch.load(os.path.join(args.savedir, "state_dict.pth"), map_location=device))
        net_fusion.eval()
    #net_fusion.cuda()
    print("Done")
        ...
        for filename in filelist_test:
            ...
            with torch.no_grad():
                ...
                for pts, features, indices in t:
                    ...
                    #features_nc = torch.ones(features.shape[0], features.shape[1], 1).cuda()
                    #pts = pts.cuda()
                    features_nc = torch.ones(features.shape[0], features.shape[1], 1)
