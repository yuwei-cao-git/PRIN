...
def main():
    ...
    ##### TEST
        ...
        for filename in filelist_test:
            ...
            scores = nearest_correspondance(pts_src.astype(np.float32), xyzrgb.astype(np.float32), scores, K=16)
            ...
            # saving labels
            save_fname = os.path.join(args.savedir, "results", f"k32_{filename}")
            scores = scores.argmax(1)
            np.savetxt(save_fname,scores,fmt='%d')

            if args.savepts:
                save_fname = os.path.join(args.savedir, "results", f"{filename}_pts_k32.txt")
