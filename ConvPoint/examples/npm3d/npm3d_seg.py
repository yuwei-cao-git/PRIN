...
def main():
    ...
    if args.restore:
        #net.load_state_dict(torch.load(os.path.join(args.savedir, "state_dict_2.pth"), map_location=device))
        state = torch.load(os.path.join(args.savedir, "state_dict.pth"), map_location=device)
        matched_weights = load_state_with_same_shape(net, state)
        model_dict = net.state_dict()
        model_dict.update(matched_weights)
        net.load_state_dict(model_dict)
        print("Restore pre-trained model...")
    if args.test:
        net.load_state_dict(torch.load(os.path.join(args.savedir, "state_dict" +args.test_epoch+ ".pth"), map_location=device))
    ...
    if not args.test:
        ...
        for epoch in range(args.epochs):
            ...
            # save the model
            torch.save(net.state_dict(), os.path.join(root_folder, "state_dict_{}.pth".format(epoch)))
        ...
        for filename in filelist_test:
            ...
            if args.savepts:
                save_fname = os.path.join(args.savedir, "results", f"{filename}_pts{args.test_epoch}.txt")
                print('saveing file: ' + save_fname)
                xyzrgb = np.concatenate([xyzrgb, np.expand_dims(scores,1)], axis=1)
                np.savetxt(save_fname,xyzrgb,fmt=['%.4f','%.4f','%.4f','%d'])
                print('output point cloud saved at ' + args.savedir)
